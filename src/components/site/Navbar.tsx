...
export function Navbar() {
  ...
  return (
    <motion.header
      ...
      className={`fixed inset-x-0 top-0 z-50 pointer-events-none flex flex-col items-center transition-all duration-500 ${
        scrolled ? "pt-3" : "pt-0"
      }`}
    >
      <div
        className={`flex items-center justify-between w-full transition-all duration-500 ease-out pointer-events-auto ${
          scrolled
            ? "max-w-7xl mx-4 md:mx-8 rounded-full border border-white/10 glass-strong shadow-2xl px-6 py-2.5"
            : "max-w-full rounded-none border-b border-white/5 bg-card/10 backdrop-blur-md px-8 md:px-16 py-4"
        }`}
      >
        <Link to="/" className="flex items-center gap-2 group">
          <Logo className={`transition-all duration-500 ${scrolled ? "h-10 md:h-12" : "h-14 md:h-16"}`} />
        </Link>
      ...
      {/* Mega menu & Mobile menu container (relative to header, but we want them to have pointer-events-auto) */}
      <div className="w-full max-w-7xl px-4 relative pointer-events-auto">
        {/* Mega menu */}
        <AnimatePresence>
          {mega && (
            <motion.div
              ...
              {mega === "products" ? (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {productCategories.map((c) => (
                    <Link key={c.key} to="/products" className="group rounded-xl p-4 hover:bg-muted transition-colors">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {industries.map((i) => (
                    <Link key={i.slug} to="/industries/$slug" params={{ slug: i.slug }} className="rounded-xl p-4 hover:bg-muted transition-colors">
          ...
          {open && (
            ...
            >
              <div className="flex flex-col">
                {navItems.map((i) => (
                  <Link key={i.to} to={i.to} className="px-3 py-2.5 rounded-lg text-sm hover:bg-muted">
                    {i.label}
                  </Link>
                ))}
      ...
      {/* Scroll progress */}
      <motion.div style={{ scaleX: progress }} className="origin-left fixed top-0 left-0 right-0 h-px bg-gradient-to-r from-brand-cyan via-brand-blue-2 to-brand-green z-50 pointer-events-none" />
    </motion.header>
  );
}
